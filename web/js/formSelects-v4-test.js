...
(function (layui, window, factory) {
	...
	Common.prototype.one = function (target) {
        ...
        // 双击事件绑定
        $(target ? target : document).off('dblclick', 'dl.' + DL).on('dblclick', 'dl.' + DL, function (e) {
            clearTimeout(clickTimeId);
            console.log("鼠标双击");
            var othis = $(e.target);
            if (othis.is('.' + LINKAGE) || othis.parents('.' + LINKAGE)[0]) {
                //linkage的处理
                console.log('进入double click了');
                ...
                var selectedNum=$(".xm-select-label").find('span');
                var firstSelectedValue=$(".xm-select-label").find('span').attr("value");
                if (selectedNum.length==0){
                    _this11.handlerLabel(_id, null, isAdd, val);
				} else {
                    // 判断选择的是否是同一个级别下的行政区划
                    var judgeCondition=firstSelectedValue.split('/').length;
                    var count=val.value.split('/').length;
                    // console.log(count);
                    if(count==judgeCondition){
                        _this11.handlerLabel(_id, null, isAdd, val);
                    }else {
                        alert("请选择属于同一行政级别的数据");
                    }
				}
