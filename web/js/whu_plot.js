...
$(document).ready(function() {
	...
	var el = document.getElementById('featureLayerSort');
	var sortable = Sortable.create(el,{
		onEnd: function (evt) {
			templateNow = getTemplate();
			var layerCount = map.graphicsLayerIds.length-1;//数量减一
			var length = evt.item.parentNode.childNodes.length;//父类一共有几个子节点
			for(i=0;i<length;i++){
				var liIndex = evt.item.parentNode.childNodes[i].firstChild.attributes[2].value;
				var liData = templateNow.featureLayer.modules[liIndex].data;
				if(liData===""){
					return;
				}
				require(["esri/map"], function(Map) {
					try {
                        var layer = map.getLayer(liData)
                        map.reorderLayer(layer,length-i-1)
                    }catch (e) {
			...
			var pointLayer = map.getLayer("pointPlotLayer");
			var lineLayer = map.getLayer("linePlotLayer");
			var polygonLayer = map.getLayer("polygonPlotLayer");
			if(pointLayer){
                map.reorderLayer(pointLayer,layerCount-1);
			}
			if(lineLayer){
                map.reorderLayer(lineLayer,layerCount-2);
			}
			if(polygonLayer){
                map.reorderLayer(polygonLayer,layerCount-3);
			}
		},
	});
	require([
